...
### Fixed
- **`/api/state` empty reply** — Rewrote `handleState()` with 16 individual try-catch blocks
  - Every Mindustry field access (`p.dead()`, `u.type.name`, `u.health`, etc.) is now individually protected
  - Outer try-catch returns `{"status":"error"}` JSON instead of crashing
  - Same protection applied to `handlePlayer()` (11 try blocks)
- **Removed all external dependencies** — No more `org.json`, `nanohttpd`, `java.util.logging`
  - Pure Java Socket HTTP server (Java 25 compatible)
  - StringBuilder-based JSON serialization
  - Resolves `java.util.logging.Logger` crash on Java 25
- **`Trigger.update` removed** — `Events.run(Trigger.update, ...)` doesn't exist in v157
  - Actions now execute synchronously

### Changed
- `build.gradle` — Removed nanohttpd and org.json dependencies
- `mod.json` — `minGameVersion` set to 154 (was 146)
